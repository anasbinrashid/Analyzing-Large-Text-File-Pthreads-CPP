...
#define numberofthreads 8 // Define the number of threads to be created
...
{
    ofstream outputFile("3affinityoutput.txt"); // Open output file
    ...
    vector<pair<string, int>> sortedWordCounts(overallfrequency.begin(), overallfrequency.end());
    ...
    cout << "Results written to 3affinityoutput.txt" << endl;
}
...
int main() 
{
    ...
    cpu_set_t cpuset;
    CPU_ZERO(&cpuset);
    sched_getaffinity(0, sizeof(cpu_set_t), &cpuset); // Get affinity for the current process
    ...
    for (int i = 0; i < numberofthreads; i++) 
    {
        ...
        cpu_set_t thread_cpuset;
        CPU_ZERO(&thread_cpuset);
        CPU_SET(i % num_cores, &thread_cpuset); // Pin thread to CPU core (i % num_cores)
        
        if (pthread_setaffinity_np(threads[i], sizeof(cpu_set_t), &thread_cpuset) != 0) 
        {
            cerr << "Failed to set CPU affinity for thread " << i << endl;
        }
    }
    ...
    {
        wordcount = wordcount + pair.second;
    }
